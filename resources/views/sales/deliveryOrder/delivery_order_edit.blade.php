...
@section('konten')
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive p-3">
            <form action="{{ route('delivery-order.update', $model->id) }}" method="POST" id="postForm"
                            ...
                            <div class="col-6 mb-3">
                                ...
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="text" name="delivery_order_code" id="delivery_order_code"
                                        class="form-control" value="{{ $model->delivery_order_code }}">
                                </div>
                                ...
                                <select name="shipping_id" id="shipping_id" class="form-select select2"
                                    ...
                                    @foreach ($shipping as $shipping)
                                        <option value="{{ $shipping->id }}"
                                            {{ $model->shipping_id == $shipping->id ? 'selected' : '' }}>
                                            {{ $shipping->nama }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="nav-align-left mb-4">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="navs-pills-left-profile" role="tabpanel">
                                @include('sales.deliveryOrder.part.info_delivery_order_edit')
                ...
                @foreach ($model->details as $detail)
                    {
                        ...
                        // TAMBAHKAN BARIS INI (Sesuaikan nama relasi tabel Sales Order Anda)
                        // Contoh jika relasi ke sales order detail menggunakan method salesOrderDetail atau salesOrder:
                        'order_code': '{{ $detail->salesOrderDetail && $detail->salesOrderDetail->salesOrder ? $detail->salesOrderDetail->salesOrder->sales_order_code : '' }}',
                    }
                    {{ !$loop->last ? ',' : '' }}
                @endforeach
            @endif
