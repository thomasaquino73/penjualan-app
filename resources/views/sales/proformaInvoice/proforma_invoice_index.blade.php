        ...
        <div
            ...
            <div class="col-12 col-lg-5">
                <div
                    ...
                    @canany(['proforma_invoice-create'])
                        <a href="{{ route('proforma-invoice.create') }}" class="btn btn-sm btn-primary">
                            <i class="ti ti-plus me-1"></i> Add Data
                        </a>
                    @endcanany
                    @canany(['proforma_invoice-trash'])
                        <a href="{{ route('proforma-invoice.trash') }}" class="btn btn-sm btn-secondary">
                            <i class="ti ti-trash me-1"></i> Trash Bin
                        </a>
                    @endcanany

                    @canany(['proforma_invoice-delete'])
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = new DataTable('#table', {
                ...
                ajax: {
                    url: '{{ route('proforma-invoice.index') }}',
                    data: function(d) {
            ...
            $(document).on('click', '.btn-processing', function() {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "This Sales Invoice will be submitted for processing!",
                    icon: 'warning',
                    showCancelButton: true,
            ...
            $(document).on('click', '.btn-processing-pr', function() {
                ...
                $.ajax({
                    url: '/proforma-invoice/change-status/' + id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
