        ...
        <div class="col-sm-6 col-lg-3 mb-4">
            <div class="card card-border-shadow-success">
                <div class="card-body">
                    ...
                    <p class="mb-1">Estimate Income</p>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="col-12 col-lg-5">
                <div
                    ...
                    @canany(['sales_quotation-create'])
                        <a href="{{ route('sales-quotation.create') }}" class="btn btn-sm btn-primary">
                            <i class="ti ti-plus me-1"></i> Add Data
                        </a>
                    @endcanany
                    @canany(['sales_quotation-trash'])
                        <a href="{{ route('sales-quotation.trash') }}" class="btn btn-sm btn-secondary">
                            <i class="ti ti-trash me-1"></i> Trash Bin
                        </a>
                    @endcanany

                    @canany(['sales_quotation-delete'])
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = new DataTable('#table', {
                ...
                ajax: {
                    url: '{{ route('sales-quotation.index') }}',
                    data: function(d) {
            ...
            $('#deleteSelected').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '/sales-quotation/delete-multiple',
                            type: 'POST',
                            data: {
                                ids: ids,
                                _token: '{{ csrf_token() }}'
            ...
            $(document).on('click', '.btn-submit-pr', function() {
                let id = $(this).data('id');
                let url = "{{ route('sales-quotation.submit', ':id') }}".replace(':id', id);

                Swal.fire({
                    title: 'Are you sure?',
                    text: "This Sales Quotation will be submitted for processing!",
                    icon: 'warning',
                    showCancelButton: true,
            ...
            $(document).on('click', '.btn-processing-pr', function() {
                ...
                $.ajax({
                    url: '/sales-quotation/change-status/' + id,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
